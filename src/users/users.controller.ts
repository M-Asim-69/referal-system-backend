...
export class UsersController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Aggregated stats for current user',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Paginated list of direct referrals',
  })
  ...
  @Patch('withdraw-password')
  @ApiOperation({
    summary: 'Set/update withdrawal password',
    description:
      'Sets the password required to submit a withdrawal request. Minimum 8 characters.',
  })
  @ApiResponse({ status: 200, description: 'Password saved' })
  @ApiResponse({ status: 400, description: 'Validation error' })
  setWithdrawPassword(
    @CurrentUser() user: User,
    @Body() dto: SetWithdrawPasswordDto,
  ) {
    return this.usersService.setWithdrawPassword(user.id, dto.password);
  }
}
